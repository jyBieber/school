<template>
  <el-aside class="index-aside" height="100vh" width="210px">
    <div class="index-aside-inner menulist" style="height:100%">
      <div v-for="item in menuList" :key="item.roleName" v-if="role==item.roleName" class="menulist-item" style="height:100%;broder:0;background-color:#425066	">
        <div class="menulistImg" style="backgroundColor:#41555d;padding:30px 0" v-if="false && menulistStyle == 'vertical'">
        ...
        <el-menu mode="vertical" :unique-opened="true" class="el-menu-demo" style="height:100%;" background-color="#425066	" text-color="#ffffff" active-text-color="#FFFFFF" default-active="0">
...
<!--          </el-submenu>-->
          ...
          <el-menu-item v-for="(menuItem, index) in item.backMenu" :key="index" :index="(index + 2).toString()" @click="menuHandler(menuItem.tableName)">
            <template slot="title">
              <!-- 图标逻辑：使用 icons 数组，确保长度足够 -->
              <i v-if="icons[index + 2]" :class="icons[index + 2]" />
      ...
      this.$nextTick(()=>{
        document.querySelectorAll('.menulist .el-menu-item').forEach(el=>{
          el.addEventListener("mouseenter", e => {
            e.stopPropagation()
            el.style.backgroundColor = "rgba(140, 156, 181, 1)"
          })
          el.addEventListener("mouseleave", e => {
            e.stopPropagation()
            el.style.backgroundColor = "#425066	"
          })
