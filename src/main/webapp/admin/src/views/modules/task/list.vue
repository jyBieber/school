        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '任务状态' : ''">
                        <el-select v-model="searchForm.taskStatusTypes" placeholder="请选择任务状态">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                               v-for="(item,index) in taskStatusTypesSelectSearch"
                               v-bind:key="index"
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        <el-button
                                v-if="isAuth('task','新增') && $storage.get('role')!== '跑腿'"
                                type="success"
                                icon="el-icon-plus"
                                @click="addOrUpdateHandler()">新增</el-button>
                        &nbsp;
                        <el-button
                                v-if="isAuth('task','删除') && $storage.get('role') == '管理员'"
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('task','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/campuserrands/upload/taskMuBan.xls"
                        >批量导入任务信息数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('task','导入导出')"
                                ...
                                :on-success="taskUploadSuccess"
                                :on-error="taskUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('task','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入任务信息数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('task','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "task.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                                     ...
                                     label="操作">
                        <template slot-scope="scope">
                            <el-button v-if="isAuth('task','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="(isAuth('task','修改') && ($storage.get('role') != '跑腿' && userId == scope.row.yonghuId) || $storage.get('role') === '管理员') && scope.row.taskStatusTypes == 1"  type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="(isAuth('task','删除') && ($storage.get('role') != '跑腿' && userId == scope.row.yonghuId) || $storage.get('role') === '管理员') && scope.row.taskStatusTypes == 1"  type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
                            <el-button v-if="isAuth('task','接单') && $storage.get('role') == '跑腿' && scope.row.taskStatusTypes == 1" type="primary" icon="el-icon-edit" size="mini" @click="orderreceiving(scope.row.id)">接单</el-button>
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :"task",//当前表表名,
                        sumColum : 'task_number', //求和字段
            ...
            getDataList() {
                ...
                params['taskDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
				...
				this.$confirm(`确定要接这个任务吗操作?`, "提示", {
					...
					this.$http({
					    ...
					    if(data && data.code === 0){
							let params = {
								taskId:id,
								orderreceivingStatusTypes:1
							}
							this.$http({
							    url: "orderreceiving/save",
							    method: "post",
							    data: params
							}).then(({data}) => {
							    if(data && data.code === 0){
            ...
            // 导入功能上传文件成功后调用导入方法
            taskUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "task/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
