        ...
        <div v-if="showFlag">
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('errands','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/campuserrands/upload/errandsMuBan.xls"
                        >批量导入跑腿数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('errands','导入导出')"
                                ...
                                :on-success="errandsUploadSuccess"
                                :on-error="errandsUploadError"
                        ...
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('errands','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "errands.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="errandsPhoto"
                                     header-align="center"
                                     width="200"
                                     label="头像">
                        <template slot-scope="scope">
                            <div v-if="scope.row.errandsPhoto">
                                <img :src="scope.row.errandsPhoto" width="100" height="100">
                            </div>
                            <div v-else>无图片</div>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   ...
                                   label="邮箱">
                        <template slot-scope="scope">
                            {{scope.row.errandsEmail}}
                        </template>
                    </el-table-column>

                    <el-table-column width="400" :align="contents.tableAlign"
                                     header-align="center"
                                     label="操作">
                        <template slot-scope="scope">
                            <el-button v-if="isAuth('errands','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('errands','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('errands','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>

                            <el-button v-if="isAuth('errands','修改')" type="success" icon="el-icon-tickets" size="mini" @click="resetPassword(scope.row.id)">重置密码</el-button>
                        </template>
                    </el-table-column>
                </el-table>
                <el-pagination
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :"errands",//当前表表名,
                        sumColum : 'errands_number', //求和字段
            ...
            getDataList() {
                ...
                params['errandsDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
            ...
            resetPassword(id) {
                //重置密码
                this.$http({
                    url: "errands/resetPassword?id=" + id,
                    method: "get",
                    // id:id
                }).then(({data}) => {
                    if(data && data.code === 0){
            ...
            // 导入功能上传文件成功后调用导入方法
            errandsUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "errands/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
